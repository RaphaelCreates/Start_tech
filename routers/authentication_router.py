...
async def login_for_access_token(
    form_data: Annotated[OAuth2PasswordRequestForm, Depends()], db: SessionDep) -> jwt_schema.Token:

    user = jwt_repo.authenticate_user(db, form_data.username, form_data.password)
    if not user:
