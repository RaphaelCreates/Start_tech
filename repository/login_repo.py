...
def create_user(db: SessionDep, name: str, totvs_id: str, password: str):
    ...
    new_user = login_model.User(name=name, totvs_id=totvs_id, password=hashed_password)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return new_user
...
def get_user_by_id(db: SessionDep, totvs_id: str):
    user = db.get(login_model.User.totvs_id, totvs_id)
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    return user

def get_all_users(db: SessionDep):
    users = db.exec(select(login_model.User)).all()
    return users
...
def update_user(id: int, db: SessionDep, request: login_model.User):
    db_user = db.get(login_model.User, id)
    if not db_user:
